...
class CAVMAE(nn.Module):
    ...
    def __init__(self, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        self.decoder_pred_a = nn.Linear(decoder_embed_dim, 256, bias=True)  # decoder to patch
        self.decoder_pred_v = nn.Linear(decoder_embed_dim, patch_size ** 2 * in_chans, bias=True)  # decoder to patch


############################################################
#        print(f"===============>>>>> projection head A: {self.decoder_pred_a}")
#        print()
#        print(f"===============>>>>> projection head v: {self.decoder_pred_v}")
...
############################################################
    ...
    def sincos_pos_embed(self,max_sequence_length = int, embedding_dim=int):
    ...
    def forward_encoder(self, a, v, mask_ratio_a, mask_ratio_v, mask_mode='unstructured',info=False):
        if info:
            verbose_enc=True
            print()
            print(print('*'*50))
            ...
            print("a"*20)
            print(f"\t\t ==> step 1.1: raw audio shape: {a.shape}")
        # embed patches
        a = a.unsqueeze(1) # from [1, 1024, 128] to [1, 1, 1024, 128]
        a = a.transpose(2, 3) # from  [1, 1, 1024, 128] to [1, 1, 128, 1024]
        a = self.patch_embed_a(a) # from [1, 1, 128, 1024] to [1, 512, 768]
        if info:
            print(f"\t\t ==> step 1.2: patch embdding: {a.shape}")
        a = a + self.pos_embed_a
        a = a + self.modality_a
        #print(f"average of audio: {torch.mean(self.modality_a)}")

        if info:
            print(f"\t\t ==> step 1.3: position/modality added: {a.shape}")
        
        if info:
            print("v"*20)
            print(f"\t\t ==> step 2.1: raw video shape: {v.shape}")
        v = self.patch_embed_v(v)
        if info:
            print(f"\t\t ==> step 2.2: patch embdding: {v.shape}")
        v = v + self.pos_embed_v
        v = v + self.modality_v
        #print(f"average of video: {torch.mean(self.modality_v)}")
        if info:
            print(f"\t\t ==> step 2.3: position/modality added: {v.shape}")

        # by default, we always use unstructured masking
        if info:
            print(f"\t\t ==> step 3: Masking")
        if mask_mode == 'unstructured':
            
            a, mask_a, ids_restore_a = self.random_masking_unstructured(a, mask_ratio_a)
            a_bare = a
            if info:
                print(f"\t\t ==> step 3.1: audio after masking: {a.shape}")
        ...
        v_bare = v
        if info:
            print(f"\t\t ==> step 3.2: video after masking: {v.shape}")
        # audio and visual stream, independent blocks
        if info:
            print(f"\t\t ==> step 4: Transformers")
        i=-1
        for blk in self.blocks_a:
            i+=1
            a = blk(a)
            if info:
                print(f"\t\t ==> step 4: audio after masking: {a.shape, i}")
        ...
        for blk in self.blocks_v:
            ...
            v = blk(v)
            if info:
                print(f"\t\t ==> step 4: video after masking: {v.shape,iv}")

        x = torch.cat((a, v), dim=1)
        if info:
            print(f"\t\t ==> step 5: audio and video concatenated: {x.shape}")
        # unified stream, shared blocks_u, but independent normalization layers
        for blk in self.blocks_u:
            x = blk(x)
        x = self.norm(x)
        if info:
            print(f"\t\t ==> step 6: x is given to unified transformers: {x.shape}")
        for blk in self.blocks_u:
            ca = blk(a, 'a')
        ca = self.norm_a(ca)
        ...
        if info:
            print(f"\t\t ==> step 7: x is returned: {x.shape}")
        return x, a_bare,mask_a, ids_restore_a, v_bare,mask_v, ids_restore_v, ca, cv

    def forward_decoder(self, x, mask_a, ids_restore_a, mask_v, ids_restore_v, info=False):
        ...
        if info:
            ...
            print("a"*20)
            print(f"\t\t ==> step 1: x is just given: {x.shape}")
        x = self.decoder_embed(x)
        if info:
            print(f"\t\t ==> step 2: Linear projection, lower dimensionality: {x.shape}")
        ...
        if info:
            print(f"\t\t ==> step 6: audio and video pos embed concatenated: {decoder_pos_embed.shape}")
        x = x + decoder_pos_embed
        if info:
            print(f"\t\t ==> step 7: position emb added to x : {x.shape}")
        ...
        if info:
            print(f"\t\t ==> step 8: modality added to x: {x.shape}")

        # apply Transformer blocks
        if info:
            print(f"\t\t ==> step 9: x is given to decoder transformers : {x.shape}")
        for blk in self.decoder_blocks:
            x = blk(x)
        x = self.decoder_norm(x)
        if info:
            print(f"\t\t ==> step 10: x after transformers : {x.shape}")

        # predictor projection
        x_a = self.decoder_pred_a(x[:, :self.patch_embed_a.num_patches, :])
        x_v = self.decoder_pred_v(x[:, self.patch_embed_a.num_patches:, :])
        if info:
            print(f"\t\t ==> step 11: predictor projection for x_audio and x_video respectively: {x_a.shape, x_v.shape}")
            
        if info:
            print(f"\t\t ==> step 12: audio and video tokens are returnd.")
    ...
    def forward_mae_loss(self, input, pred, mask, modality):
        if modality == 'a':
            ...
            input = input.unsqueeze(1) # [1, 1, 1024, 128]
            input = input.transpose(2, 3) # [1, 1, 128, 1024]
    ...
    def forward(self, audio, imgs, mask_ratio_a=0.75, mask_ratio_v=0.75, mae_loss_weight=1., contrast_loss_weight=0.01, mask_mode='unstructured', info=False):
        # latent is used for reconstruction (mae), latent_c_{a,v} are used for contrastive learning
        info=info
        #x, a_bare,mask_a, ids_restore_a, v_bare,mask_v, ids_restore_v, ca, cv
        latent,a_bare, mask_a, ids_restore_a,v_bare, mask_v, ids_restore_v, latent_c_a, latent_c_v = self.forward_encoder(audio, imgs, mask_ratio_a, mask_ratio_v, mask_mode=mask_mode,info=info)
        # if mae loss is used
        # print(latent.shape,       mask_a.shape,        ids_restore_a.shape, mask_v.shape,        ids_restore_v.shape)
        # torch.Size([1, 177, 768]) torch.Size([1, 512]) torch.Size([1, 512]) torch.Size([1, 196]) torch.Size([1, 196])
        if mae_loss_weight != 0:
            pred_a, pred_v = self.forward_decoder(latent, mask_a, ids_restore_a, mask_v, ids_restore_v,info=info)
            # print(pred_a.shape,         pred_v.shape)
            # (torch.Size([1, 512, 256]), torch.Size([1, 196, 768]))
            loss_mae_a = self.forward_mae_loss(audio, pred_a, mask_a, 'a') # audio: torch.Size([1, 1024, 128]), pred_a: torch.Size([1, 512, 256])
            loss_mae_v = self.forward_mae_loss(imgs, pred_v, mask_v, 'v') # video: torch.Size([1, 3, 224, 224])), pred_v: torch.Size([1, 196, 768])
        ...
        return loss, loss_mae, loss_mae_a, ids_restore_a, loss_mae_v, ids_restore_v, loss_c, mask_a, mask_v, c_acc, pred_a, pred_v, a_bare, v_bare
...
class CAVMAEFT(nn.Module):
    ...
    def forward(self, a, v, mode,info=False):
        ...
        if mode == 'multimodal':
            if info:
                ...
                print(print('*'*50))
                print("a"*20)
                print(f"\t\t ==> step 1: raw audio shape: {a.shape}")
            a = a.unsqueeze(1)
            a = a.transpose(2, 3)
            if info:
                print(f"\t\t ==> step 2: ready for patch embdding: {a.shape}")
            a = self.patch_embed_a(a)
            if info:
                print(f"\t\t ==> step 3: after patch embdding: {a.shape}")
            a = a + self.pos_embed_a
            a = a + self.modality_a
            if info:
                print(f"\t\t ==> step 4: position/modality are added: {a.shape}")

            if info:
                print('v'*20)
                print(f"\t\t ==> step 1: raw video shape: {v.shape}")
                print(f"\t\t ==> step 1: video ready for patch embedding")
            v = self.patch_embed_v(v)
            if info:
                print(f"\t\t ==> step 2: after patch embedding: {v.shape}")
            v = v + self.pos_embed_v
            v = v + self.modality_v
            if info:
                print(f"\t\t ==> step 3: position/modality are added: {v.shape}")

            if info:
                print("a"*20)
                print(f"\t\taudio is ready to be fed to transformers")
            for blk in self.blocks_a:
                a = blk(a)

            if info:
                print("v"*20)
                print(f"\t\tvideo is ready to be fed to transformers")
            for blk in self.blocks_v:
                v = blk(v)
            if info:
                print("u"*20)
                print(f"\t\taudio and video are concatenated")
            x = torch.cat((a, v), dim=1)
            if info:
                print(f"\t\tnow fed to a unified transformers with 1 attention head. Shape: {x.shape}")
            for blk in self.blocks_u:
                x = blk(x)
            if info:
                print(f"\t\tx given to norm layer. Shape: {x.shape}")
            x = self.norm(x)
            if info:
                print(f"\t\tMax: {torch.max(x)}, Min: {torch.min(x)}")
                print(f"\t\tafter norm layer. Shape: {x.shape}")

            x = x.mean(dim=1)
            if info:
                print(f"\t\tMax: {torch.max(x)}, Min: {torch.min(x)}")
                print(f"\t\tx is just calculated by means, and ready for MLP. Shape: {x.shape}")
            x = self.mlp_head(x)
            if info:
                print(f"\t\tMax: {torch.max(x)}, Min: {torch.min(x)}")
                print(f"\t\tFinally after MLP: {x.shape}")
                print()
                print(print('*'*50))
            return x
